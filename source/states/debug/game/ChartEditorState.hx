	...
	override function create()
	{
		...
		for (animData in Note._noteFile.animationData)
		{
			if (animData.indices != null && animData.indices.length > 0)
				noteSelector.animation.addByIndices(animData.name, animData.prefix, animData.indices, "", Std.int(animData.fps), animData.looped);
			else
				noteSelector.animation.addByPrefix(animData.name, animData.prefix, Std.int(animData.fps), animData.looped);
		}
...
class TopBarText extends EditorText
{
	...
	override public function new(text:String, action:Void->Void):Void
	{
		...
		overlapBox.setGraphicSize(Std.int(Math.max(64, this.width)), Std.int(24));
...
class ChartNote extends FlxSprite
{
	...
	private function playAnim():Void
	{
		for (animData in Note._noteFile.animationData)
		{
			...
			{
				animation.addByIndices(animData.name, animData.prefix, [0], "", Std.int(animData.fps), animData.looped);
