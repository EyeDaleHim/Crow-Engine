			...
			if (animArray != null)
			{
				...
				for (anim in animArray)
				{
					if (anim.indices.length > 0) {
						var fps:Int = Std.int(anim.fps);
                        animation.addByIndices(anim.name, anim.prefix, anim.indices, "", fps, anim.looped);
					} else {
						animation.addByPrefix(anim.name, anim.prefix, Std.int(anim.fps), anim.looped); }
				}
			}
			else
			{
